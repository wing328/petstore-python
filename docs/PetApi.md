...
body = new Pet() # Pet | Pet object that needs to be added to the store (optional)
...
api_instance = swagger_client.PetApi
status = array(available) # list[str] | Status values that need to be considered for query (optional) (default to available)
...
api_instance = swagger_client.PetApi
tags = NULL # list[str] | Tags to filter by (optional)
...
file = new file() # file | file to upload (optional)
